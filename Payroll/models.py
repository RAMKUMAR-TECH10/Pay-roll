...
class ProductionLog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    employee_id = db.Column(db.Integer, db.ForeignKey('employee.id'), nullable=False)  # Worker who made it
    supervisor_id = db.Column(db.Integer, db.ForeignKey('employee.id'), nullable=True)  # Supervisor overseeing
    ...
    employee = db.relationship('Employee', foreign_keys=[employee_id], backref=db.backref('produced_bundles', lazy='dynamic'))
    supervisor = db.relationship('Employee', foreign_keys=[supervisor_id], backref=db.backref('supervised_production', lazy='dynamic'))
...
class Employee(db.Model):
    ...
    employment_type = db.Column(db.String(20), default='permanent')  # permanent, contract, temporary
    ...
    salary_records = db.relationship('Salary', backref='employee', lazy='dynamic', cascade='all, delete-orphan')
...
class Attendance(db.Model):
    ...
    status = db.Column(db.String(20), default='present')  # present, absent, late, half-day, sick, leave
...
class Salary(db.Model):
    ...
    __table_args__ = (
        ...
        db.CheckConstraint('gross_salary >= 0', name='check_gross_salary_positive'),
        db.CheckConstraint('bonus >= 0', name='check_bonus_positive'),
        db.CheckConstraint('deductions >= 0', name='check_deductions_positive'),
        db.CheckConstraint('tax >= 0', name='check_tax_positive'),
        db.CheckConstraint('net_salary >= 0', name='check_net_salary_positive'),
    )
